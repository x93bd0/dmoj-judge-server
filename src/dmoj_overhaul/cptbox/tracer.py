...
BAD_SECCOMP = sys.platform == "linux" and tuple(
    map(int, os.uname().release.partition("-")[0].split("."))
) < (4, 8)
...
class TracedPopen(Process):
    ...
    def __init__(
        ...
        *,
        ...
        if BAD_SECCOMP:
            raise RuntimeError(
                f"Sandbox requires Linux 4.8+ to use seccomp, you have {os.uname().release}"
            )
        ...
        self._callbacks = [
            [None] * MAX_SYSCALL_NUMBER for _ in range(PTBOX_ABI_COUNT)
        ]
        if security is None:
    ...
    def wait(self) -> int:
        ...
        if not self.was_initialized:
            if self.returncode == PTBOX_SPAWN_FAIL_NO_NEW_PRIVS:
                ...
                raise RuntimeError(
                    "process failed to initialize with unknown exit code: %d"
                    % self.returncode
                )
    ...
    def kill(self) -> None:
        ...
        if self.returncode is None:
            ...
            log.warning(
                "Skipping the killing of process because it already exited: %s",
                self.pid,
            )
    ...
    def _callback(self, syscall) -> bool:
        if self.debugger.abi == PTBOX_ABI_INVALID:
            log.warning(
                "Received invalid ABI when handling syscall %d", syscall
            )
            return False
    ...
    def __init_streams(self, stdin, stdout, stderr) -> None:
        ...
        self.stdin_needs_close = self.stdout_needs_close = (
            self.stderr_needs_close
        ) = False
