...
from ._cptbox import AT_FDCWD, Debugger
from .filesystem_policies import (
    ExactFile,
    FilesystemAccessRule,
    FilesystemPolicy,
    RecursiveDir,
)
from .handlers import ACCESS_EFAULT, ACCESS_EPERM, ALLOW
...
class CompilerIsolateTracer(IsolateTracer):
        ...
        self.update(
            {
                ...
                sys_mkdir: self.handle_file_access(
                    FilesystemSyscallKind.WRITE, file_reg=0
                ),
                sys_mkdirat: self.handle_file_access_at(
                    FilesystemSyscallKind.WRITE, dir_reg=0, file_reg=1
                ),
                sys_rmdir: self.handle_file_access(
                    FilesystemSyscallKind.WRITE, file_reg=0
                ),
                # Linking system calls
                sys_link: self.handle_file_access(
                    FilesystemSyscallKind.WRITE, file_reg=1
                ),
                sys_linkat: self.handle_file_access_at(
                    FilesystemSyscallKind.WRITE, dir_reg=2, file_reg=3
                ),
                sys_unlink: self.handle_file_access(
                    FilesystemSyscallKind.WRITE, file_reg=0
                ),
                sys_unlinkat: self.handle_file_access_at(
                    FilesystemSyscallKind.WRITE, dir_reg=0, file_reg=1
                ),
                sys_symlink: self.handle_file_access(
                    FilesystemSyscallKind.WRITE, file_reg=1
                ),
                # Miscellaneous other filesystem system calls
                sys_chdir: self.handle_file_access(
                    FilesystemSyscallKind.READ, file_reg=0
                ),
                sys_chmod: self.handle_file_access(
                    FilesystemSyscallKind.WRITE, file_reg=0
                ),
                ...
                sys_fsync: ALLOW,
                sys_fadvise64: ALLOW,
                sys_fchmodat: self.handle_file_access_at(
                    FilesystemSyscallKind.WRITE, dir_reg=0, file_reg=1
                ),
            ...
            self.update(
                {
                    ...
                    sys_cap_enter: ALLOW,
                    sys_utimes: self.handle_file_access(
                        FilesystemSyscallKind.WRITE, file_reg=0
                    ),
                }
            )
    ...
    def handle_rename(self, debugger: Debugger) -> None:
        self.access_check(
            self._write_fs_jail_getter, self._dirfd_getter_cwd, file_reg=0
        )(debugger)
        self.access_check(
            self._write_fs_jail_getter, self._dirfd_getter_cwd, file_reg=1
        )(debugger)

    def handle_renameat(self, debugger: Debugger) -> None:
        self.access_check(
            self._write_fs_jail_getter,
            self._dirfd_getter_from_reg(0),
            file_reg=1,
        )(debugger)
        self.access_check(
            self._write_fs_jail_getter,
            self._dirfd_getter_from_reg(2),
            file_reg=3,
        )(debugger)
    ...
    def do_utimensat(self, debugger: AdvancedDebugger) -> None:
        ...
        if times_ptr:
            try:
                ...
                raise DeniedSyscall(
                    ACCESS_EFAULT, f"Cannot read from times_ptr: {times_ptr}"
                )
            ...
            if debugger.uarg0 != AT_FDCWD and not debugger.uarg1:
                ...
                if not self.write_fs_jail.check(path):
                    raise DeniedSyscall(
                        ACCESS_EPERM, f"Denying access to {path}"
                    )
            else:
                return self.handle_file_access_at(
                    FilesystemSyscallKind.WRITE, dir_reg=0, file_reg=1
                )(debugger)
