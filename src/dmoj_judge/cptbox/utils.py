...
from ._cptbox import memfd_create, memfd_seal
...
class MmapableIO(io.FileIO, metaclass=ABCMeta):
    def __init__(
        self, fd, *, prefill: Optional[bytes] = None, seal=False
    ) -> None:
        super().__init__(fd, "r+")
...
MemoryIO = next(
    (i for i in (MemfdIO, UnnamedFileIO, NamedFileIO) if i.usable_with_name())
)
