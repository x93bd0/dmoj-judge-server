...
with (
    open("linux-x86.tbl", "w") as x86,
    utf8reader(urlopen(LINUX_SYSCALLS_32)) as data,
):
    for line in data:
...
with (
    open("linux-x64.tbl", "w") as x64,
    open("linux-x32.tbl", "w") as x32,
...
with (
    open("linux-arm.tbl", "w") as arm,
    utf8reader(urlopen(LINUX_SYSCALLS_ARM)) as data,
):
    id = 0
    for line in data:
...
with (
    open("linux-generic.tbl", "w") as generic64,
    open("linux-generic32.tbl", "w") as generic32,
...
with (
    open("freebsd.tbl", "w") as output,
    utf8reader(urlopen(FREEBSD_SYSCALLS)) as data,
):
    for line in data:
