...
from ._cptbox import AT_FDCWD, Debugger, bsd_get_proc_cwd, bsd_get_proc_fdno
from .filesystem_policies import FilesystemAccessRule, FilesystemPolicy
...
from ._utils.unicode import utf8text
...
class IsolateTracer(dict):
    def __init__(
        ...
        *,
        ...
        if sys.platform.startswith("freebsd"):
            ...
            self._getfd_pid = lambda pid, fd: os.readlink(
                "/proc/%d/fd/%d" % (pid, fd)
            )
        ...
        self.update(
            {
                # Deny with report
                sys_openat: self.handle_openat(
                    dir_reg=0, file_reg=1, flag_reg=2
                ),
                sys_open: self.handle_open(file_reg=0, flag_reg=1),
                sys_faccessat: self.handle_file_access_at(
                    FilesystemSyscallKind.READ, dir_reg=0, file_reg=1
                ),
                sys_faccessat2: self.handle_file_access_at(
                    FilesystemSyscallKind.READ, dir_reg=0, file_reg=1
                ),
                sys_access: self.handle_file_access(
                    FilesystemSyscallKind.READ, file_reg=0
                ),
                sys_readlink: self.handle_file_access(
                    FilesystemSyscallKind.READ, file_reg=0
                ),
                sys_readlinkat: self.handle_file_access_at(
                    FilesystemSyscallKind.READ, dir_reg=0, file_reg=1
                ),
                sys_stat: self.handle_file_access(
                    FilesystemSyscallKind.READ, file_reg=0
                ),
                sys_stat64: self.handle_file_access(
                    FilesystemSyscallKind.READ, file_reg=0
                ),
                sys_lstat: self.handle_file_access(
                    FilesystemSyscallKind.READ, file_reg=0
                ),
                sys_lstat64: self.handle_file_access(
                    FilesystemSyscallKind.READ, file_reg=0
                ),
            ...
            self.update(
                {
                    ...
                    sys_realpathat: self.handle_file_access_at(
                        FilesystemSyscallKind.READ, dir_reg=0, file_reg=1
                    ),
                }
            )
    ...
    def _compile_fs_jail(
        self, fs: Sequence[FilesystemAccessRule]
    ) -> FilesystemPolicy:
    ...
    def _fs_jail_getter_from_kind(
        self, kind: FilesystemSyscallKind
    ) -> FSJailGetter:
        def getter(debugger: Debugger) -> FilesystemPolicy:
            return {
    ...
    def handle_file_access(
        self, kind: FilesystemSyscallKind, *, file_reg: int
    ) -> AccessChecker:
        return self.access_check(
            self._fs_jail_getter_from_kind(kind),
            self._dirfd_getter_cwd,
            file_reg=file_reg,
        )

    def handle_file_access_at(
        self, kind: FilesystemSyscallKind, *, dir_reg: int, file_reg: int
    ) -> AccessChecker:
        return self.access_check(
            self._fs_jail_getter_from_kind(kind),
    ...
    def handle_openat(
        self, *, dir_reg: int, file_reg: int, flag_reg: int
    ) -> AccessChecker:
        return self.access_check(
            self._fs_jail_getter_from_open_flags_reg(flag_reg),
            self._dirfd_getter_from_reg(dir_reg),
    ...
    def handle_fstat(self, *, dir_reg: int, file_reg: int) -> AccessChecker:
        def check(debugger: Debugger) -> None:
            ...
            full_path = self.get_full_path_unnormalized(
                debugger, rel_file, dirfd=dirfd
            )
    ...
    def access_check(
        self,
        fs_jail_getter: FSJailGetter,
        dirfd_getter: DirFDGetter,
        *,
        file_reg: int,
    ) -> AccessChecker:
        def check(debugger: Debugger) -> None:
    ...
    def get_rel_file(self, debugger: Debugger, *, reg: int) -> str:
        ...
        try:
            ...
            raise DeniedSyscall(
                ACCESS_ENAMETOOLONG, f"Overly long path: {e.args[0]}"
            )
    ...
    def get_full_path_unnormalized(
        self, debugger: Debugger, rel_file: str, *, dirfd: int
    ) -> str:
    ...
    def get_dir(self, debugger: Debugger, *, dirfd: int) -> str:
        dirfd = (dirfd & 0x7FFFFFFF) - (
            dirfd & 0x80000000
        )  # Interpret dirfd as a signed 32-bit integer
        if dirfd == AT_FDCWD:
            return self._getcwd_pid(debugger.tid)
        return self._getfd_pid(debugger.tid, dirfd)

    def _access_check(
        self, debugger: Debugger, file: str, fs_jail: FilesystemPolicy
    ) -> None:
        ...
        if normalized.startswith("/proc/self"):
            file = os.path.join(
                f"/proc/{debugger.tid}", os.path.relpath(file, "/proc/self")
            )
        ...
        try:
            same = (
                normalized == real
                or real.startswith("/memfd:")
                or os.path.samefile(projected, real)
            )
        except OSError:
            raise DeniedSyscall(
                ACCESS_ENOENT,
        ...
        if not fs_jail.check(normalized):
            raise DeniedSyscall(
                ACCESS_EACCES, f"Denying {file}, normalized to {normalized}"
            )
        ...
        if normalized != real:
            ...
            if not real.startswith("/memfd:") and not fs_jail.check(real):
                raise DeniedSyscall(
                    ACCESS_EACCES, f"Denying {file}, real path {real}"
                )
    ...
    def handle_kill(self, debugger: Debugger) -> None:
        ...
        if target != debugger.pid:
            raise DeniedSyscall(
                ACCESS_EPERM,
                f"Cannot kill other processes (target={target}, self={debugger.pid})",
            )
    ...
    def handle_prlimit(self, debugger: Debugger) -> None:
        ...
        if target not in (0, debugger.pid):
            raise DeniedSyscall(
                ACCESS_EPERM,
                f"Cannot prlimit other processes (target={target}, self={debugger.pid})",
            )
    ...
    def handle_prctl(self, debugger: Debugger) -> None:
        ...
        if debugger.arg0 not in (
            ...
            raise DeniedSyscall(
                protection_fault,
                f"Non-whitelisted prctl option: {debugger.arg0}",
            )
